...
author:
  ...
  - affiliation: The Sainsbury Laboratory, University of East Anglia, Norwich Research Park, Norwich, UK, NR4 7UH, dan.maclean@tsl.ac.uk
abstract: ""
geometry: margin=1in
biblio-style: apsr
...
fig_caption <- captioner::captioner(prefix = "Figure ")
fig_caption("effectorcollection", "Workflow diagram for collection of effector sequences from the PHI-Base database annotation and cross reference to UniProt")

fig_caption("noneffectorcollection", "Workflow diagram for collection of secreted non-effector sequences from Ensembl Bacteria, Fungi, and Protists")

fig_caption("modelarch", "Overview of model architectures tested in this analysis, A: CNN-LSTM model; B: CNN-GRU model; C: LSTM-Embedding model; D: GRU-Embedding model")

fig_caption("trainingprogress", "Training trajectories showing Accuracy and Loss over 40 epochs for four model types on A: Bacterial sequence training set; B: Fungal sequence training set; C: Oomycete sequence training set")

fig_caption("overallmodel", "Final performance of models in each taxon on the unseen, hold-out test data fraction of sequences.")

fig_caption("confusionmodelcorrs", "Confusion matrices showing tendency of models to call false positive or false negative errors on the final test data; and Pearson correlations between classifications made by different models. A: Models trained on bacterial sequences; B: Models trained on fungal sequences; C: Models trained on oomycete sequences.")

fig_caption("ensembleresults", "Final performance of Ensemble models in each taxon on the unseen, hold-out test data fraction of sequences.")

fig_caption("vsothersmetrics", "Accuracy and F1 score of the classifications made by our models compared with those made by widely used software")

fig_caption("vsotherscfmat", "Confusion matrices of classifications made by our models compared with those made by widely used software. A: Tools evaluated on bacterial sequences; B: Tools evaluated on fungal sequences; C: Tools evaluated on oomycete sequences.")

fig_caption("owndata", "Performance of models on training and validation datasets used to train other effector prediction tools. A: Tools trained on bacterial sequences; B: Tools trained on fungal sequences; C: Tools trained on oomycete sequences.")

fig_caption("activations", "Grad-CAM activation profiles of CNN-LSTM models  when run with all datasets showing the region of the sequence the model is most able to use to classify (leftmost panel). The most active part of the sequences are shown in logo plots of sequences extracted from the activation region within the dotted lines for effector data (centre panel) and non-effector data (rightmost panel). A: Models trained on bacterial sequences; B: Models trained on fungal sequences; C: Models trained on oomycete sequences.")


tbl_caption <- captioner::captioner(prefix = "Table ")
tbl_caption("effsummary", 'Count of effectors listed in publications curated by PHI-Base used in this study in three major plant pathogen groups')

tbl_caption("aclossfinal", 'Accuracy and loss values from best performing parameters values for each model')

tbl_caption("phyperaatype", 'Enrichment analysis of amino acid types. Proportions of each amino acid type in the activation region of All Sequences : Effector Sequences, with (Probability). Probability by hypergeometric test of the observed number of amino acid type in the region of activation in effector sequences in our compiled sequence set relative to a background of the same region in all sequences in that taxon.')
```
...
```{r trainingprogress, fig.height= 10, fig.width= 10, fig.cap=fig_caption("trainingprogress",display="cite")}
...
```{r overallmodel, fig.cap=fig_caption("overallmodel",display="cite")}
# apa_table(
...
```{r vsothersmetrics, fig.width=8, fig.height=6, fig.cap=fig_caption("vsothersmetrics",display="cite"), fig.pos="H"}
...
```{r vsotherscfmat, fig.width=8, fig.height=8, fig.cap=fig_caption("vsotherscfmat",display="cite"), fig.pos="H"}
...
```{r, owndata, fig.height = 8, fig.cap=fig_caption("owndata",display="cite")}
d <- readr::read_csv(here::here("data","processed", "owndatasets.csv"))  
  ...
  mkplot <- function(df, which_tax ) {
    ...
    geom_bar(stat= 'identity', fill = rgb(52/255, 106/255, 141/255), colour = rgb(52/255, 106/255, 141/255)) +
...
```{r activations, fig.height=10, fig.width=10, fig.cap=fig_caption("activations",display="cite")}
...
The datasets generated and/or analysed during the current study are available in the 'ruth-effectors-prediction' repository, https://github.com/TeamMacLean/ruth-effectors-prediction. Individual datasets location within this repository are listed per dataset in the Results section
...
RKR and DM were supported by The Gatsby Charitable Foundation core grant to The Sainsbury Laboratory.

## Authors' Contributions

RKR carried out experiments and developed analysis code, and developed the software tools. DM designed experiments, interpreted results, and wrote the manuscript. All authors have read and approved the manuscript.
...
# Figure Legends
`r fig_caption("effectorcollection", display="full")`

`r fig_caption("noneffectorcollection", display="full")`

`r fig_caption("modelarch", display="full")`

`r fig_caption("trainingprogress", display="full")`

`r fig_caption("overallmodel",display="full")`

`r fig_caption("confusionmodelcorrs",display="full")`

`r fig_caption("ensembleresults",display="full")`

`r fig_caption("vsothersmetrics",display="full")`

`r fig_caption("vsotherscfmat",display="full")`

`r fig_caption("owndata",display="full")`

`r fig_caption("activations",display="full")`

# Table Legends
`r tbl_caption("effsummary", display="full")`

`r tbl_caption("aclossfinal", display="full")`

`r tbl_caption("phyperaatype",display="full")`
