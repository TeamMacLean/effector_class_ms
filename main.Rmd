...
We trained four model types on each taxon specific sequence set: CNN-LSTM, CNN-GRU, LSTM-Embedding,  GRU-Embedding. We trained each model using a basic random hyperparameter setting initialisation step followed by All training was performed in R 3.5.3, Tensorflow 1.12.0, Keras 2.2.4, Python 3.6.9 on a MacBook Pro (Retina, 13-inch, Early 2013) with an Intel Core i7-3450M (3.00GHz) processor and 8 GB (1600 MHz DDR3) of RAM. ^[Ruth, what about the stuff you did on the cluster with the GPUs? This isn't correct, is it?]. 

### Hyperparameter Scans

Hyperparameter scans were performed in ^[Ruth, can you outline the packages and methods in scikit-learn/tensorflow that you used for this] Code for this can be found in [https://github.com/TeamMacLean/ruth-effectors-prediction/tree/master/scripts/python-scripts/hyperparameter-scan-scripts](https://github.com/TeamMacLean/ruth-effectors-prediction/tree/master/scripts/python-scripts/hyperparameter-scan-scripts). All modle training was performed as described in section Model Training.

### Fine tuning

Fine tuning was performed manually using Tensorflow/scikit learn ^[Ruth, can you add the software and versions your scripts used here] Scripts implementing the tuning can be found at  [https://github.com/TeamMacLean/ruth-effectors-prediction/tree/master/scripts/python-scripts/manual_tune_scripts](https://github.com/TeamMacLean/ruth-effectors-prediction/tree/master/scripts/python-scripts/manual_tune_scripts). 
...
We defined a range of values for the hyperparameters that could be optimised in each architecture, 10 for CNN-LSTM, 12 for CNN-GRU, 9 for LSTM-Embedding and 9 for GRU-Embedding. To test all combinations of values in these ranges would take a prohibitive amount of processor time, so we selected 50 sets of values for each model in each taxon at random to start training, `r 120 * 50` models in total. ^[Ruth, can you confirm that this interpretation is correct]  Model variants within the hyperparameter search were assessed by comparing accuracy values on the development validation fraction of the training data. Other hyperparameters were fixed and are listed in Table \@ref(tab:fixedhyperparam) ^[Ruth, please can you add the actual values that were used to this table, as in table \@ref(tab:possiblecombination)] For each model type and taxon training data combination we selected the hyperparameter set giving highest accuracy on the validation fraction, the best performing values can be see in Table \@ref(tab:possiblecombination). From this we had twelve candidate models to develop further. 
