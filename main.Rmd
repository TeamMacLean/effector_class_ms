...
We are also interested in evaluating positive datasets from other tools on both our model and on theirs, to be able to have a more unbiased comparison of the models' performance. Table \@ref(tab:accf1scoreeffectorpdata) shows the of accuracy and F1 score values of classification results of three different positive datasets from EffectorP 1.0 and 2.0 which can be found in [http://effectorp.csiro.au./data.html](http://effectorp.csiro.au./data.html). As it can be seen, EffectorP tends perform better on their training data compared to the validation data, which indicates that EffectorP is likely overfitted to the traning data which makes it less generalised.

The accuracy and F1 score values of evaluating positive dataset of EffectiveT3 obtained from [https://effectors.csb.univie.ac.at/sites/eff/files/others/TTSS_positive_training.faa](https://effectors.csb.univie.ac.at/sites/eff/files/others/TTSS_positive_training.faa) and positive datasets of DeepT3 obtained from [https://github.com/lje00006/DeepT3/tree/master/DeepT3/DeepT3-Keras/data](https://github.com/lje00006/DeepT3/tree/master/DeepT3/DeepT3-Keras/data) are shown in Table
\@ref(tab:accf1scoreeffectivet3data) and  \@ref(tab:accf1scoredeept3data), respectively. Our ensemble weighted bacteria model performs better in classifing effector than using both EffectiveT3 for plant-associated bacteria and for all bacteria (see Table \@ref(tab:accf1scoreeffectivet3data)). Similar case with DeepT3 datasets, overall as shown in Table \@ref(tab:accf1scoredeept3data), our ensemble model for bacteria improved the performance of DeepT3. 

Table \@ref(tab:accf1scoreeffectRdata) shows the results of examining several reference RxLR effectors (PexRD36, PexRD1, ipi01/Avrblb1, Avr1, Avr4, and Avr3a) used by EffectR [@effectR] on both our CNN-LSTM model and EffectR. It is shown that our model is able to recognise around 64% of the oomycete effector sequence data without having any prior knowledge of effector motifs as it is required in EffectR. 
...
apa_table(
  ...
  digits = 4,
  align = 'c',
  caption = 'Accuracy and F1-Score from prediction results of EffectorP data evaluated on our model and both EffectorP 1.0 and EffectorP 2.0',
  placement = "H"
)
```

```{r accf1scoreeffectivet3data}
apa_table(
  readr::read_csv("data/processed/acc_f1score_result_effectivet3_data_prediction.csv"),
  digits = 4,
  align = 'c',
  caption = 'Accuracy and F1-Score from prediction results of EffeciveT3 positive data evaluated on our model and EffectiveT3 trained on both plant associated protein and all protein.',
  placement = "H"
)
```

```{r accf1scoredeept3data}
apa_table(
  readr::read_csv("data/processed/acc_f1score_result_deept3_data_prediction.csv"),
  digits = 4,
  align = 'c',
  caption = 'Accuracy and F1-Score from prediction results of three different DeepT3 positive datasets evaluated on our model and DeepT3.',
  placement = "H"
)
```

```{r accf1scoreeffectRdata}
apa_table(
  readr::read_csv("data/processed/acc_f1_score_effectR_data_prediction.csv"),
  digits = 4,
  align = 'c',
  caption = 'Accuracy and F1-Score from prediction results of reference sequences of EffectR evaluated on our model and EffectR.',
  placement = "H"
)
```


```{r vsothersmetrics, fig.width=8, fig.height=4, fig.cap="Accuracy and F1 score of the classifications made by our models compared with those made by widely used software", fig.pos="H"}
...
```{r effectorpdatacfmat, fig.width=6, fig.height=6, fig.cap="Confusion matrices of classifications made by our fungi model compared with EffectorP 1.0 and EffectorP 2.0 evaluated on both training and validation EffectorP data.", fig.pos="H", message=FALSE, include=FALSE}
...
```{r effectivet3datacfmat, fig.width=6, fig.height=3, fig.cap="Confusion matrices of classifications made by our bacteria model compared with both EffectiveT3 based on plant associated and all protein evaluated on positive EffectiveT3 data.", fig.pos="H", message=FALSE, include=FALSE}

readr::read_csv("data/processed/conf_mat_effectivet3_data_prediction.csv") %>% 
  dplyr::mutate(
  tool = factor(tool, levels = c("ensemble_weighted", "effectivet3_all", "effectivet3_plant"))
  ) %>% 
  ggplot2::ggplot() +
  ggplot2::aes(x = Truth, y = Prediction) +
  ggplot2::geom_tile(ggplot2::aes(fill = Freq), colour = "white") +
  ggplot2::geom_text(ggplot2::aes(label = Freq), vjust = 0.5) +
  ggplot2::facet_grid(~tool) +
  ggplot2::scale_fill_viridis_c(begin = 0.2, end = 1) +
  ggplot2::labs(x = "True value", y = "Prediction") +
  ggplot2::coord_fixed()
```
...
```{r deept3datacfmat, fig.height=8, fig.cap="Confusion matrices of classifications made by our bacteria model compared with DeepT3 evaluated on positive EffectiveT3 data.", fig.pos="H", message=FALSE, include=FALSE}

readr::read_csv("data/processed/conf_mat_deept3_data_prediction.csv") %>% 
  dplyr::mutate(
  tool = factor(tool, levels = c("ensemble_weighted", "deept3"))
  ) %>% 
  ggplot2::ggplot() +
  ggplot2::aes(x = Truth, y = Prediction) +
  ggplot2::geom_tile(ggplot2::aes(fill = Freq), colour = "white") +
...
```{r effectRdatacfmat, fig.width=6, fig.height=3, fig.cap="Confusion matrices of classifications made by our bacteria model compared with both EffectiveT3 based on plant associated and all protein evaluated on positive EffectiveT3 data.", fig.pos="H", message=FALSE, include=FALSE}

readr::read_csv("data/processed/conf_mat_effectR_data_prediction.csv") %>% 
  dplyr::mutate(
  tool = factor(tool, levels = c("cnn_lstm", "effectR"))
  ) %>% 
  ggplot2::ggplot() +
  ggplot2::aes(x = Truth, y = Prediction) +
  ggplot2::geom_tile(ggplot2::aes(fill = Freq), colour = "white") +
